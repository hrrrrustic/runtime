...
namespace System.Text.Json
{
    public sealed partial class Utf8JsonWriter
    {
        ...
        /// <summary>
        /// Writes the input as JSON content. It is expected that the input content is a single complete JSON value with size less than <see cref="int.MaxValue"/>.
        /// </summary>
        /// <param name="utf8Json">The raw JSON content to write.</param>
        /// <param name="skipInputValidation">Whether to validate if the input is an RFC 8259-compliant JSON payload.</param>
        /// <exception cref="ArgumentException">Thrown if the length of the input is zero or greater or equal to <see cref="int.MaxValue"/>.</exception>
        /// <exception cref="JsonException">
        /// Thrown if <paramref name="skipInputValidation"/> is <see langword="false"/>, and the input
        /// is not a valid, complete, single JSON value according to the JSON RFC (https://tools.ietf.org/html/rfc8259)
        /// or the input JSON exceeds a recursive depth of 64.
        ...
        public void WriteRawValue(ReadOnlySequence<byte> utf8Json, bool skipInputValidation = false)
        {
            ...
            WriteRawValueCore(utf8Json, (int)len, skipInputValidation);
        }
        ...
        private void WriteRawValueCore(ReadOnlySequence<byte> utf8Json, int length, bool skipInputValidation)
        {
            if (length == 0)
            {
            ...
            int maxRequired = length + 1; // Optionally, 1 list separator. We've guarded against integer overflow earlier in the call stack.
